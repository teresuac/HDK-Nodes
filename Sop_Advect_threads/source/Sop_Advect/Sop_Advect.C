...
public:
	THREADED_METHOD2(                   // Construct two parameter threaded method
		...
		int, substep,            // An integer parameter named p1
		float, amp)				// A float parameter named p2
...
void VOLADVECT::advectPartial(int substep, float amp, const UT_JobInfo &info)
{
	...
	sizex =  (int)(voxel_sizeb[0]);
	sizey =   (int)(voxel_sizeb[1]);
	sizez =   (int)(voxel_sizeb[2]);
...
void newSopOperator(OP_OperatorTable *table)
{
	...
	op = new OP_Operator(
    		"volume_a_threads",                  // internal name, needs to be unique in OP_OperatorTable (table containing all nodes for a network type - SOPs in our case, each entry in the table is an object of class OP_Operator which basically defines everything Houdini requires in order to create nodes of the new type)
    		"Volume A threads",                  // UI name
...
OP_ERROR SOP_VolAdvect::cookMySop(OP_Context &context)
{
	...
	//UT_VoxelArrayWriteHandleF volume_handleW;
	VOLADVECT voladvect_multithread;
	...
	GA_FOR_ALL_PRIMITIVES( gdp, prim)
	{
		if (prim->getTypeId() == GA_PRIMVOLUME)		
		{
			...
			if (((first_volume_name.compare(vol_advect_name)) ==0) )
			{
				...
				voladvect_multithread.volume_data = volume;
				voladvect_multithread.volume_handleW = volume->getVoxelWriteHandle();
				UT_Vector3 voxel_sizeb(voladvect_multithread.volume_handleW->getRes(0), voladvect_multithread.volume_handleW->getRes(1), voladvect_multithread.volume_handleW->getRes(2) );
	...
	{	
		...
		if (prim->getTypeId() == GA_PRIMVOLUME)
		{
			...
			if (volume_name.compare(vel_x)==0)
			{
				...
				voladvect_multithread.volume_velx  = volume_b;
			...
			if (volume_name.compare(vel_y)==0)
			{
				...
				voladvect_multithread.volume_vely = volume_b;
			 ...
			 if (volume_name.compare(vel_z)==0)
			{
				...
				voladvect_multithread.volume_velz = volume_b;
	...
	for (int idx_z = 0; idx_z < voxel_size[2]; ++idx_z)
	{
	 ...
	 if (vols_found == 4)
	 {
		 voladvect_multithread.advect(sub, amp);
	 }
